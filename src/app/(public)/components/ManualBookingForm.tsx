...
export function ManualBookingForm({ services, businessHours, createBookingAction, fetchSlotsAction }:ManualBookingFormProps) {
    ...
    // Renamed state to bookedSlots and initialized to empty array
    ...
    const timeSlots = useMemo(() => {
        ...
        // Assuming businessHours[0] is the relevant hours for now
        // In a real app, you'd check hours for the selectedDate's day of the week
    ...
    const loadBookedSlots = useCallback(async () => {
        ...
        try {
            ...
            // Call the server action to fetch booked slots
            const bookedData = await fetchSlotsAction(selectedService, dateString);
            
            // Map the returned start_time into the HH:MM format used in timeSlots
   ...
   const handleSubmit = async () => {
        ...
        try {
            ...
            // This Date constructor overload (Y, M, D, H, M, S) is crucial. It creates 
            // a Date object representing the time in the browser's local timezone (e.g., 18:00:00 IST).
            const localStartDate = new Date(
                date.getFullYear(), 
                date.getMonth(), // Date.getMonth() is 0-indexed (0=Jan, 11=Dec)
                date.getDate(), 
                hour, 
                minute, 
                0
            ...
            // .toISOString() correctly converts the local time (e.g., 18:00 IST) 
            // to the corresponding UTC time (e.g., 12:30 UTC) for database storage.
            const startTimeStr = localStartDate.toISOString(); // e.g., "2025-11-27T12:30:00.000Z"
            ...
            formData.append('startTime', startTimeStr); // Pass the correctly calculated UTC ISO string
    ...
    // --- JSX (NO CHANGES NEEDED BELOW, BUT INCLUDED FOR COMPLETENESS) ---
