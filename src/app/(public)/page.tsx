...
import { Card, CardDescription, CardHeader, CardContent, CardTitle } from "@/app/components/ui/card";
...
export default async function ServiceSelectionPage({ onSelectService }: ServicesPageProps) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-100 to-pink-100 rounded-full px-4 py-2 mb-4">
            <span className="text-sm bg-gradient-to-r from-orange-600 to-pink-600 bg-clip-text text-transparent">Premium Services</span>
          </div>
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Choose from our range of expert-led fitness programs designed to help you achieve your goals.
          </p>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <Card key={service.id} className={`overflow-hidden border-2 hover:border-transparent transition-all duration-300 hover:shadow-xl ${gradientClasses[index].shadow} group`}>
              <div className="aspect-video relative overflow-hidden">
                ...
                />
                <div className={`absolute top-4 right-4 bg-gradient-to-r ${gradientClasses[index].gradient} text-white px-3 py-1 rounded-full text-sm shadow-lg`}>
              ...
              <CardHeader>
                <div className="mb-2">
                  <CardTitle className="text-xl text-black mb-1">{service.name}</CardTitle>
                  <div className="text-sm text-gray-500">{service.duration_minutes}</div>
                </div>
              ...
              <CardContent>
                <ul className="space-y-2 mb-6">
                  {service.features?.map((feature, idx) => (
                    <li key={idx} className="flex items-center text-sm text-gray-600">
                      <div className={`p-0.5 rounded-full bg-gradient-to-r ${gradientClasses[index].gradient} mr-3 flex-shrink-0`}>
                ...
                <ServiceCardButton
                  ...
                  gradient={gradientClasses[index].gradient}
                  shadow={gradientClasses[index].shadow}
        ...
        <div className="text-center mt-16 bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl p-8 border border-gray-200">
          ...
          <Button variant="outline" className="border-2 border-black text-black hover:bg-black hover:text-white transition-all duration-300 hover:scale-105">
